...
## writing function to put workers into array


def workerSeparator(name):
    global listOfWorkers
    i0 = name.index("[")
    i1 = name.index("]")
    s0 = name[:i0]
    s1, s2 = name[i0+1:i1].split("-")
    n0 = int(s1)
    n1 = int(s2)
...
def step2():
    ...
    script = textwrap.dedent(
        ...
        """)
    hostexecute(script, manager)
    results = Host.ssh(hosts=manager, command=f'''/sbin/ifconfig eth0 | grep 'inet' | cut -d: -f2''')
    print(Printer.write(results))
    for entry in results:
        print(str(entry["stdout"]))
...
def step4():
    ...
    for worker in listOfWorkers:
        ...
        while not success:
            ...
            for entry in results:
                if 'Could not connect to' in str(entry["stdout"]):
                    msg = "The SLURM script could not install needed packages, but will try again. " \
                          "Please consult the Known Issues section of the documentation."
                    banner(msg)
                    # return msg
                    success = False
            time.sleep(10)
    ...
    banner("After successful reboot, ssh back into manager and test slurm by issuing $ srun --nodes=3 hostname (change 3 to number of nodes if necessary)")
    os.system("cms host reboot "+hosts)
