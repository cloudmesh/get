...
def step2():
    ...
    script = textwrap.dedent(
        ...
        /sbin/ifconfig eth0 | grep 'inet' | cut -d: -f2
        """)
    hostexecute(script, manager)
    for entry in results:
        print(str(entry["stdout"]))
...
def step4():
    ...
    for worker in listOfWorkers:
        ...
        while not success:
            ...
            for entry in results:
                if 'Could not connect to' in str(entry["stdout"]):
                    msg = "The SLURM script could not install needed packages. Please consult the Known Issues section of the documentation."
                    banner(msg)
                    # return msg
                    success = False
            time.sleep(10)
